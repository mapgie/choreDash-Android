...
class ChorePayloadTest {
    ...
    fun `a chore edit without a date leaves the due date columns out`() {
        ...
        val payload = chorePatch("Vacuum", "Cleaning", null, ChoreRepeat(7, RepeatUnit.DAY), dueDate = null, includeSchedule = false)
    ...
    fun `a yearly chore with a date writes the date, the unit and 365 days`() {
        val payload = chorePatch(
            ...
            ChoreRepeat(1, RepeatUnit.YEAR), LocalDate.of(2026, 10, 1), includeSchedule = false,
        )
    ...
    fun `clearing a chore's date sends explicit nulls`() {
        val payload = chorePatch("House insurance", null, null, repeat = null, dueDate = null, includeSchedule = true)
    ...
    fun `a repeat in days never names the repeat unit column on its own`() {
        val payload = chorePatch("Water plants", null, null, ChoreRepeat(3, RepeatUnit.DAY), dueDate = null, includeSchedule = false)
