...
- **Theme:** Four built-in Material 3 palettes (Mist default, Sage, Coral, Teal in `ui/theme/Color.kt`) plus a custom theme with per-role colour pickers and background overrides; light/dark/system brightness and a WCAG high-contrast toggle (`DashTheme` in `ui/theme/Theme.kt`). Body and UI text use the system default font (`ui/theme/Type.kt`) — no brand font.
